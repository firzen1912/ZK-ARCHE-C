// ==============================
// c_server.c  (DESIGN A: TOFU pin server pubkey + KEY CONFIRMATION MACs)
// ==============================
//
// This server is compatible with the Rust Design A server:
//
// - Persistent server identity (server_sk.bin) => server_pub sent during SETUP + AUTH.
// - Registry stores device_id -> device_pub in registry.bin.
// - AUTH performs:
//     * verify client Schnorr
//     * send server Schnorr proof
//     * derive session key
//     * send "server finished" tag_s
//     * verify client "client finished" tag_c
//
...
//     C->S: A(32) | s(32)    (Schnorr PoP)
//
//   MSG_AUTH  = 0x02
//     C->S: 0x02 | device_id(32) | A_c(32) | s_c(32) | nonce_c(32) | eph_c(32)
//     S->C: server_pub(32) | A_s(32) | s_s(32) | nonce_s(32) | eph_s(32) | tag_s(32)
//     C->S: tag_c(32)
//
// Transcript encoding (no Merlin):
...
//
// Build:
//   gcc -O2 -std=c11 -Wall -Wextra c_server.c -o c_server -lsodium
//
#include <sodium.h>
#include <stdio.h>
#include <stdint.h>
...
// Format: repeated (device_id 32 bytes || pubkey 32 bytes)
// -----------------------------
typedef struct {
    uint8_t id[32];
    uint8_t pub[32];
...
// Upsert with idempotency: if device_id exists, pub must match.
// On new insert, immediately persist registry.bin.
...
static void tr_append(transcript_t *tr, const char *label, const uint8_t *val, uint32_t vlen) {
    ...
    // NOTE: This buffer is fixed-size (4096). Keep messages small.
...
// -----------------------------
// Schnorr verify/prove (Ristretto255)
// Verify: s*G == A + c*X
//
// IMPORTANT:
// libsodium's crypto_scalarmult_ristretto255() returns -1 if the input point
// is invalid. Always check the return value.
// -----------------------------
static int schnorr_verify_setup(const uint8_t device_id[32], const uint8_t pubkey[32],
    ...
    // cX = c * pubkey   (check for invalid pubkey point)
    if (crypto_scalarmult_ristretto255(cX, c, pubkey) != 0) {
        return -1;
    }
...
static int schnorr_verify_auth(const uint8_t device_id[32], const uint8_t expected_pub[32],
    ...
    if (crypto_scalarmult_ristretto255(cX, c, expected_pub) != 0) {
        return -1;
    }
...
// Server prove (Design A): bind pubkey + A + nonce_s + eph_s (no SERVER_ID)
...
// shared = eph_s_secret * eph_c
// salt   = nonce_c || nonce_s
// info   = "session key" || device_id || eph_c || eph_s
//
// Returns 0 on success, -1 if eph_c is invalid.
...
// Key confirmation (KC)
//
// th = SHA256( transcript("kc_v1", handshake_fields...) )
...
int main(int argc, char **argv) {
    ...
    char ip[64];
    uint16_t port;
    if (parse_bind(bind, ip, &port) != 0) {
        fprintf(stderr, "bad --bind value\n");
        return 1;
    }

    // Persistent server identity: load-or-create server_sk.bin
    ...
    for (;;) {
        ...
        if (msg_type == MSG_SETUP) {
            ...
            // Consume token bytes if present (optional feature).
            // Use a 255-byte buffer to avoid the "comparison always false" warning.
            if (tlen > 0) {
                uint8_t *tmp = (uint8_t*)malloc(tlen);
                if (!tmp) { close(cfd); continue; }
                if (recv_all(cfd, tmp, tlen) != 0) { free(tmp); close(cfd); continue; }
                free(tmp);
            }

            uint8_t device_id[32], device_pub[32];
            if (recv_all(cfd, device_id, 32) != 0) { close(cfd); continue; }
            if (recv_all(cfd, device_pub, 32) != 0) { close(cfd); continue; }

            // Challenge nonce
            uint8_t server_nonce[32];
            randombytes_buf(server_nonce, 32);

            // Send server_static_pub + server_nonce
            ...
            if (reg_upsert(&reg, &reg_n, device_id, device_pub) != 0) {
        ...
        if (msg_type == MSG_AUTH) {
            ...
            // Send: server_pub | A_s | s_s | nonce_s | eph_s | tag_s
            if (send_all(cfd, server_pub, 32) != 0) { close(cfd); continue; }
            ...
            if (send_all(cfd, tag_s, 32) != 0) { close(cfd); continue; }

            // Receive + verify client finished tag
            uint8_t tag_c[32];
            if (recv_all(cfd, tag_c, 32) != 0) { close(cfd); continue; }

            uint8_t expected_c[32];
            hmac_tag(expected_c, k_c2s, "client finished", th);
            if (sodium_memcmp(expected_c, tag_c, 32) != 0) {
                fprintf(stderr, "AUTH: key confirmation failed (tag_c mismatch)\n");
                close(cfd);
                continue;
            }
        ...
        // Unknown msg_type => close connection
        close(cfd);
    }
}