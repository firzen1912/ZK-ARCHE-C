#!/usr/bin/env bash
# zk-arche.sh — ZK-ARCHE automation script (C version)
...
${_C}BUILD${_N}
  build                                   Compile C binaries (server + client)
...
cmd_build() {
  ...
  gcc -O2 -std=c11 -Wall -Wextra "${PROJECT_ROOT}/server.c" -o "$SERVER_BIN" -lsodium
  gcc -O2 -std=c11 -Wall -Wextra "${PROJECT_ROOT}/client.c" -o "$CLIENT_BIN" -lsodium
...
cmd_status() {
  ...
  if [[ -x "$SERVER_BIN" ]]; then
    log_ok  "c_server binary: $SERVER_BIN"
  else
    log_warn "c_server binary: not built ($SERVER_BIN)"
  fi
  if [[ -x "$CLIENT_BIN" ]]; then
    log_ok  "c_client binary: $CLIENT_BIN"
  else
    log_warn "c_client binary: not built ($CLIENT_BIN)"
  fi
