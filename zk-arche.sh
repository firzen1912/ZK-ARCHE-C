...
# Builds the server and client C binaries.
# FIX: link with -lpthread for per-connection threading support.
cmd_build() {
  require_cmd gcc
  log_header "Building C binaries"

  # Avoid ETXTBUSY when an old binary is still running by linking to fresh
  # temporary paths and then atomically renaming into place.
  local server_tmp client_tmp
  server_tmp="${SERVER_BIN}.new"
  client_tmp="${CLIENT_BIN}.new"
  rm -f "$server_tmp" "$client_tmp"

  gcc -O2 -std=c11 -Wall -Wextra "${PROJECT_ROOT}/server.c" -o "$server_tmp" -lsodium -lssl -lcrypto -lpthread
  gcc -O2 -std=c11 -Wall -Wextra "${PROJECT_ROOT}/client.c" -o "$client_tmp" -lsodium -lssl -lcrypto

  mv -f "$server_tmp" "$SERVER_BIN"
  mv -f "$client_tmp" "$CLIENT_BIN"
